...
public class ChessLineupSelector {
    ...
    public ChessLineupSelector(RandomGenerator randomGenerator, ChessLineupSettings lineupSettings, SeasonSettings seasonSettings) {
        this.randomGenerator = randomGenerator;
        this.lineupSettings = lineupSettings;
        this.seasonSettings = seasonSettings;
    }

    public Player pickPlayerFromTeam(int slot, Team teamA, Team teamB, boolean selectForWhite) throws ChessMonteCarloSimulationException {
        ...
        Player selectedPlayer = selectPlayer(teamPlayerProbabilities);
        this.alreadySelectedPlayers.add(selectedPlayer);

        return selectedPlayer;
    }

    private Player selectPlayer(Map<Player, Double> lineupProbabilities) {
        ...
        EnumeratedIntegerDistribution distribution = new EnumeratedIntegerDistribution(this.randomGenerator, idxToGenerate, discreteProbabilities);
    ...
    private Player pickRandomPlayerFromTeam(Team team) {
        Player selectedPlayer = null;
        do {
            int randomPlayerIndex = ThreadLocalRandom.current().nextInt(0, team.getPlayerList().size());
            selectedPlayer = team.getPlayerList().get(randomPlayerIndex);
        } while(this.alreadySelectedPlayers.contains(selectedPlayer));
        return selectedPlayer;
    }

    private Map<Player, Double> calculateLineupProbabilities(final Map<Player, Double> lineupProbabilities, int slot, Team team, Team opponentTeam, boolean selectForWhite) throws ChessMonteCarloSimulationException {
        ...
        switch (this.lineupSettings.getLineupStrategy()) {
            ...
            case WHITE_BLACK_PERFORMANCE:
                newLineupP.replaceAll((player,p) -> {
                    ...
                    if(selectForWhite) {
                        if(deltaWinLossWhite > 0) return p + (deltaWinLossWhite * this.lineupSettings.whiteBlackPerformanceStrategyLineupInfluenceFactor);
                    } else {
                        if(deltaWinLossBlack > 0) { return p + (deltaWinLossBlack * this.lineupSettings.whiteBlackPerformanceStrategyLineupInfluenceFactor);
                        }
                    }
                    return p;
                });
            ...
            case ASCENDING_RATING_STRENGTH:
                ...
                Map.Entry<Player, Double> entryAsc = getEntryAtPosition(this.seasonSettings.getGamesPerMatch() - slot - 1, sortedDesc);
                if(entryAsc != null)
            ...
            case TRADITIONAL:
                ...
                break;
            default:
                throw new ChessMonteCarloSimulationException("No Lineup Strategy defined");
        }
        return newLineupP;
    }
    ...
    private boolean applyOptimizedLineup(Team team) {
        return this.lineupSettings.getOptimizeLineupTeamName() != null
                && this.lineupSettings.getOptimizeLineupTeamName().equals(team.getName());
    }
    ...
    private Player adjustLineupProbabilitiesBasedOnLikelyOpponent(Map<Player, Double> lineupProbabilities, Map<Player, Double> lineupProbabilitiesOpponent, boolean selectForWhite) {
        ...
        lineupProbabilities.replaceAll((player,p) -> {
            ...
            if (selectForWhite) {
                double influence = (diffWhiteBlackDelta + eloDiff) * this.lineupSettings.playerPerformanceAndEloLineupInfluenceFactor;
                return Math.max(p + influence, 0.0);
            } else {
                double influence = (diffBlackWhiteDelta + eloDiff) * this.lineupSettings.playerPerformanceAndEloLineupInfluenceFactor;
                return Math.max(p + influence, 0.0);
            }
        });
